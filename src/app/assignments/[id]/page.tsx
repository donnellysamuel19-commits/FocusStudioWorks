...
export default function AssignmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchAssignmentData = useCallback(async () => {
    ...
    toast({
      variant: 'destructive',
      title: 'Error',
      description: "Assignment not found or you don't have permission.",
    });
  ...
  const getStatusIcon = (status: StudySession['state']) => {
    switch (status) {
      ...
      case 'Active':
        return <PlayCircle className="text-blue-400 animate-pulse" />;
      case 'Pending':
        return <Hourglass className="text-yellow-400" />;
      default:
        return null;
    }
  };
    ...
    <div className="container mx-auto p-6">
          ...
          {sessions.length === 0 ? (
            ...
            <div className="space-y-4">
              {sessions.map((session) => (
                <div
                  key={session.id}
                  className="border p-4 rounded-lg flex items-center justify-between gap-4 bg-card text-card-foreground"
                >
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className="hidden sm:block">{getStatusIcon(session.state)}</div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold truncate" title={session.targetObject}>
                        {session.targetObject}
                      </p>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {session.state === 'Pending' && (
                          <>
                            <DropdownMenuItem
                              onSelect={() =>
                                router.push(`/assignments/${assignment.id}/sprint/edit/${session.id}`)
                              }
                            >
      ...
      <AlertDialog open={!!sessionToDelete} onOpenChange={(open) => !open && setSessionToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeleting} onClick={() => setSessionToDelete(null)}>
              Cancel
            </AlertDialogCancel>
